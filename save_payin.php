...
$value = trim($_POST['txt_transac']);
if(strlen($value)>0)
{
      
      if($_FILES['filepayin']['error']==0)
      {
          $imgtype=$_FILES['filepayin']['type']; 
          $type=explode("/",$imgtype);
          
            if(strtolower($type[0])==strtolower("image"))
            {
                $imagesite="payinimages/".$_FILES['filepayin']['name']; 
            ...
            else
            {
                ...
                $uploaded = move_uploaded_file($_FILES['filepayin']['tmp_name'], $destination);
                $checktransac = mysql_query("select * from reserved_room where transaction_id='$value'");
                if($uploaded)
                {
                    if(mysql_num_rows($checktransac)>0)
                    {
                        $currdate = date("Y-m-d H:i:s");
                        $sql_payin = "insert into payinslip (transactionid,paydate,payinslip) values('$value','$currdate','$destination')";
                        $insert_payin=mysql_query($sql_payin);
                        if($insert_payin)
                        {
                            $success = "1";
                        }
      ...
      elseif($_FILES['filepayin']['error']==4)
      {
             echo "<script>";
            echo "alert('กรุณาอัพโหลดสลิปการโอนเงินค่ะ');";
            echo "</script>";
      }
      else
      {
            echo "<script>";
            echo "alert('ไฟล์มีขนาดใหญ่เกินไป');";
            echo "</script>";
      }
}
else
{
    echo "<script>";
    echo "alert('กรุณาระบุรหัสการจอง');";
    echo "</script>";
}
...
{  
        ...
        $updatestatus = mysql_query("update reserved_room set status='approved' where transaction_id='$value'") ;
        ...
        if($difftime>$approvetime)
        {
               ...
               // echo "รายการจองนี้หมดเวลาชำระเงินแล้ว, โปรดติดต่อเจ้าหน้าที่เพื่อตรวจสอบข้อมูลอีกครั้งค่ะ<br>";
               // exit();
    ...
    <head>
        ...
        <script>
             ...
             function confirmpayin(id)
             {
                 ...
                 jQuery.get("admin/confirmpayin.php",{value:id},function(data){
                       
                       if(data=='1')  
                       { 
                           alert("ยืนยันการโอนเงินเรียบร้อยแล้ว");
                           jQuery("#paystatus").html("ชำระเงินแล้ว");
                            jQuery("#paystatus").css("color","blue");

                            jQuery("#btn_comfirm_payin").attr("disabled",true);  
                            jQuery(".newstatus").remove();
                       }
                 });
...
<form  name="frm_savereserved" method="post" action=""  target="">
    <table class="tbreserve">
        <tr>
            <th colspan="2" style="text-align: left;">บันทึกข้อมูลการชำระเงินสำหรับรายการ <?php echo $arrbooking['transaction_id'];?>&nbsp;&nbsp;ได้ส่งให้คุณทางอีเมล์ของคุณแล้ว</th>
        ...
        elseif($arrbooking['status']=='approved')
        {
            ?>
              <tr>
                  <td colspan="2" style="text-align: left;">คุณสามารถนำใบบันทึกรายการนี้มาแจ้งต่อเจ้าหน้าที่เพื่อเข้าใช้ห้องประชุมตามวันเวลาที่ระบุ</td>                  
              </tr>
              <?php
        }
        ?>
...
elseif($success=='-1')
{
     echo "ขออภัย ไม่พบรายการจองนี้ค่ะ";
}
?>
